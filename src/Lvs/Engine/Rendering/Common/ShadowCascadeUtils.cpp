...
NormalizedShadowSettings NormalizeShadowSettings(const ShadowQualityProfile& settings) {
    NormalizedShadowSettings normalized{};
    normalized.MapResolution = std::max(
        kShadowMinResolution,
        std::min(kShadowMaxResolution, settings.MapResolution == 0 ? kShadowDefaultMapResolution : settings.MapResolution)
    );
    normalized.CascadeResolutionScale = std::max(
        kShadowMinCascadeResolutionScale,
        std::min(kShadowMaxCascadeResolutionScale, settings.CascadeResolutionScale)
    );
    normalized.CascadeSplitLambda = std::max(0.0F, std::min(1.0F, settings.CascadeSplitLambda));
    normalized.TapCount = std::max(kShadowMinTapCount, std::min(kShadowMaxTapCount, settings.TapCount));
    normalized.BlurAmount = std::max(kShadowMinBlurAmount, std::min(kShadowMaxBlurAmount, settings.BlurAmount));
    normalized.Bias = kShadowDefaultBias;
    normalized.FadeWidth = kShadowDefaultFadeWidth;
    normalized.CascadeCount = std::max(1, std::min(kMaxShadowCascades, settings.CascadeCount));
    normalized.MaxDistance = std::max(kShadowMinDistance, std::min(kShadowMaxDistance, settings.MaxDistance));
    return normalized;
}
...
std::array<std::uint32_t, kMaxShadowCascades> ComputeCascadeResolutions(
    ...
    const double clampedScale = std::clamp(
        static_cast<double>(cascadeResolutionScale),
        static_cast<double>(kShadowMinCascadeResolutionScale),
        static_cast<double>(kShadowMaxCascadeResolutionScale)
    );
    ...
    for (int i = 0; i < kMaxShadowCascades; ++i) {
        ...
        cascadeResolutions[static_cast<std::size_t>(i)] = std::max(
            kShadowMinResolution,
            static_cast<std::uint32_t>(std::lround(std::max(static_cast<double>(kShadowMinResolution), scaledResolution)))
        );
    }
